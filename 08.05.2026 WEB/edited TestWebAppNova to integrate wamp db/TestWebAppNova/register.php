...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($inputEmail === '' || $password === '' || $confirmPassword === '') {
        ...
        $stmt = $conn->prepare('SELECT id FROM customer WHERE username = ? LIMIT 1');
        ...
        if ($stmt->num_rows > 0) {
            ...
            $stmt->close();
            $insert = $conn->prepare('INSERT INTO customer (username, password) VALUES (?, ?)');
            $insert->bind_param('ss', $inputEmail, $password);
            ...
            <form action="register.php" method="POST" autocomplete="on">
                <div class="mb-3">
                    <div class="input-group">
                        ...
                        <input type="email" name="email" class="form-control" placeholder="Email Address" required autocomplete="username" value="<?php echo htmlspecialchars($inputEmail); ?>">
                    </div>
                </div>
